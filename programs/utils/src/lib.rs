...
/// Optional trait for cache functionality - only implement if needed
...
pub trait ExtendedProofData: ProofData {
...
pub trait Executable: Sized + TypeIdentifiable {
    ...
    fn execute<T>(&mut self, stack: &mut T) -> Vec<Vec<u8>>
    where
        T: BidirectionalStack + ProofData + StarkCommitmentTrait + StarkVerifyTrait + ExtendedProofData + FullProofDataVerifier2 + CacheStorage + FullProofDataVerifier3;
