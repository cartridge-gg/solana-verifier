...
impl CachedProofData for BidirectionalStackAccount {
    ...
    fn get_fri_verify_data_and_verify_variables_mut<T: Sized, P: Sized>(&mut self) -> (&mut T, &mut P) {
        ...
        assert_eq!(verify_variables_bytes.len(), std::mem::size_of::<P>());
        let fri_verify_data = unsafe { &mut *(fri_verify_data_bytes.as_mut_ptr() as *mut T) };
        let verify_variables = unsafe { &mut *(verify_variables_bytes.as_mut_ptr() as *mut P) };
        (fri_verify_data, verify_variables)
    }
}
...
impl ExtendedProofData for BidirectionalStackAccount {
    ...
    fn get_pows_and_domains_mut(&mut self) -> (&[Felt; POWS_SIZE], &mut [Felt; DOMAINS_SIZE]) {
        unreachable!("get_pows_and_domains_mut not supported in verifier3")
    }

    fn get_global_values(&self) -> &GlobalValues {
        unreachable!("get_global_values not supported in verifier3")
    }

    fn get_global_values_mut(&mut self) -> &mut GlobalValues {
        unreachable!("get_global_values_mut not supported in verifier3")
    }

    fn set_global_values(&mut self, _global_values: GlobalValues) {
        unreachable!("set_global_values not supported in verifier3")
    }

    fn get_proof_and_global_values_mut(&mut self) -> (&StarkProof, &mut GlobalValues) {
        unreachable!("get_proof_and_global_values_mut not supported in verifier3")
    }
}
...
impl ProofDataVerification for BidirectionalStackAccount {
    ...
    fn get_sc_proof_cv<T: Sized, P: Sized>(&mut self) -> (&mut T, &mut P, &mut [Felt; COLUMN_VALUES_SIZE]) {
        ...
        let stark_commitment = unsafe { &mut *(stark_commitment_bytes.as_mut_ptr() as *mut T) };
        let proof = unsafe { &mut *(proof_bytes.as_mut_ptr() as *mut P) };
        let column_values = &mut self.column_values;
        (stark_commitment, proof, column_values)
    }
}

// Dummy implementation of CacheStorage for verifier3 (not actually used)
