...
/// Minimal state for verifier1 - same layout as UniversalStackAccount
#[repr(C)]
#[derive(Debug)]
...
// Dummy implementation of StarkVerifyTrait for verifier1 (not actually used)
...
impl ProofDataVerification for BidirectionalStackAccount {
    fn get_proof_data_references<T: Sized>(
        ...
        unreachable!("get_proof_data_references not supported in verifier1")
    }
    fn get_stark_commitment_and_proof<T: Sized, P: Sized>(&self) -> (&T, &P) {
        unreachable!("get_stark_commitment_and_proof not supported in verifier1")
    }
    fn get_stark_commitment_and_proof_mut<T: Sized, P: Sized>(&mut self) -> (&mut T, &mut P) {
        unreachable!("get_stark_commitment_and_proof_mut not supported in verifier1")
    }
    fn get_constraint_coefficients(&self) -> &[Felt; N_CONSTRAINTS] {
        &self.constraint_coefficients
    }
    ...
    fn get_sc_proof_cv<T: Sized, P: Sized>(
        ...
        unreachable!("get_sc_proof_cv not supported in verifier1")
    }
}
...
impl CachedProofData for BidirectionalStackAccount {
    ...
    fn get_fri_verify_data_and_verify_variables_mut<T: Sized, P: Sized>(
        ...
        unreachable!("get_fri_verify_data_and_verify_variables_mut not supported in verifier1")
    }
}
...
impl CacheStorage for BidirectionalStackAccount {
    fn store_in_cache<T>(&mut self, _data: &T) {
        unreachable!("CacheStorage not supported in verifier1")
    }
