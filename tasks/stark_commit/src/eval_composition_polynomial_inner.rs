...
fn compute_powers<T: BidirectionalStack + ProofDataDecommitment>(
    ...
    autogenerated_pows[20] = obj.trace_generator.pow_felt(
...
fn compute_cpu_decode_intermediates<T: BidirectionalStack + ProofDataDecommitment>(
    ...
    bits[10] =
        column_row!(mv, 0, 10) - (column_row!(mv, 0, 11) + column_row!(mv, 0, 11));
    bits[11] =
        column_row!(mv, 0, 11) - (column_row!(mv, 0, 12) + column_row!(mv, 0, 12));
    bits[12] =
        column_row!(mv, 0, 12) - (column_row!(mv, 0, 13) + column_row!(mv, 0, 13));
    bits[13] =
        column_row!(mv, 0, 13) - (column_row!(mv, 0, 14) + column_row!(mv, 0, 14));
    bits[14] =
        column_row!(mv, 0, 14) - (column_row!(mv, 0, 15) + column_row!(mv, 0, 15));
...
fn compute_cpu_decode_constraints<T: BidirectionalStack + ProofDataDecommitment>(
    ...
    let mut sum = FELT_0;
    let mut value = (bits[0] * bits[0] - bits[0])
    ...
    let value = (column_row!(mv, 1, 1)
        - (((column_row!(mv, 0, 0) * g.offset_size + column_row!(mv, 4, 4))
            * g.offset_size
            + column_row!(mv, 4, 8))
            * g.offset_size
            + column_row!(mv, 4, 0)))
    ...
    let value = (flag_res_op1_0 * flag_res_op1_0 - flag_res_op1_0)
        .field_div(&felt_nonzero!(domains[5]));
    sum += constraints_coef[4] * value;

    let value = (flag_pc_update_regular_0 * flag_pc_update_regular_0
...
fn compute_cpu_operands_constraints<T: BidirectionalStack + ProofDataDecommitment>(
    ...
    let value = (column_row!(mask_values, 5, 2)
        - bits[9] * column_row!(mask_values, 1, 9))
...
fn compute_call_ret<T: BidirectionalStack + ProofDataDecommitment>(
    ...
    let value = (bits[12]
        * (column_row!(mask_values, 1, 5)
            - (column_row!(mask_values, 1, 0) + bits[2] + FELT_1)))
        .field_div(&felt_nonzero!(domains[5]));
    ...
    let value = (bits[12]
        * (bits[12] + bits[12] + FELT_1 + FELT_1 - (bits[0] + bits[1] + FELT_4)))
        .field_div(&felt_nonzero!(domains[5]));
...
fn compute_poseidon_step3<T: BidirectionalStack + ProofDataDecommitment>(
    ...
    let value =
        (column_row!(mask_values, 5, 169) + poseidon_bits[2] + poseidon_bits[2]
            - (poseidon_bits[0]
                + poseidon_bits[1]
    ...
    let value =
        (column_row!(mask_values, 1, 971) + poseidon_bits[5] + poseidon_bits[5]
...
impl Executable for EvalCompositionPolynomialInner {
    fn execute<T: BidirectionalStack + ProofDataDecommitment>(
        ...
        msg!("EvalCompositionPolynomialInner::execute phase: {:?}", self.phase as u8);
        match self.phase {
            ...
            EvalCompositionPolynomialInnerPhase::ComputeCpuDecodeIntermediates => compute_cpu_decode_intermediates(self, stack),
            EvalCompositionPolynomialInnerPhase::ComputeCpuDecodeConstraints => compute_cpu_decode_constraints(self, stack),
            EvalCompositionPolynomialInnerPhase::ComputeCpuOperandsConstraints => compute_cpu_operands_constraints(self, stack),
            EvalCompositionPolynomialInnerPhase::ComputeCallRet => compute_call_ret(self, stack),
            EvalCompositionPolynomialInnerPhase::ComputeInitialFinal => compute_initial_final(self, stack),
            EvalCompositionPolynomialInnerPhase::ComputeMemory => compute_memory(self, stack),
            EvalCompositionPolynomialInnerPhase::ComputeRangeCheck16 => compute_range_check16(self, stack),
            EvalCompositionPolynomialInnerPhase::ComputeDilutedCheck => compute_diluted_check(self, stack),
            EvalCompositionPolynomialInnerPhase::ComputePedersenStep1 => compute_pedersen_step1(self, stack),
            EvalCompositionPolynomialInnerPhase::ComputePedersenStep2 => compute_pedersen_step2(self, stack),
            EvalCompositionPolynomialInnerPhase::ComputeRangeCheck => compute_range_check(self, stack),
            EvalCompositionPolynomialInnerPhase::ComputeBitwise => compute_bitwise(self, stack),
            EvalCompositionPolynomialInnerPhase::ComputePoseidonStep1 => compute_poseidon_step1(self, stack),
            EvalCompositionPolynomialInnerPhase::ComputePoseidonStep2 => compute_poseidon_step2(self, stack),
            EvalCompositionPolynomialInnerPhase::ComputePoseidonStep3 => compute_poseidon_step3(self, stack),
            EvalCompositionPolynomialInnerPhase::ComputePoseidonStep4 => compute_poseidon_step4(self, stack),
