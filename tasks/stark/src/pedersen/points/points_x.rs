...
impl PedersenPointsX {
    pub fn new(pedersen_point: Felt) -> Self {
        Self {
            step: PedersenPointsXPhase::Init,
            pedersen_point,
            result: Felt::ZERO,
        }
    }
...
impl Default for PedersenPointsX {
    fn default() -> Self {
        Self::new(Felt::ZERO)
    }
}

impl Executable for PedersenPointsX {
    fn execute<T: BidirectionalStack + ProofData + StarkCommitmentTrait>(
        &mut self,
        stack: &mut T,
    ) -> Vec<Vec<u8>> {
        match &self.step {
            PedersenPointsXPhase::Init => {
                println!("Computing Pedersen points X...");
                let point = self.pedersen_point;
                let mut res = FELT_0;
                res = res * point
                ...
                self.step = PedersenPointsXPhase::ComputePointStep1;
                self.result = res;
                vec![]
            }
            PedersenPointsXPhase::ComputePointStep1 => {
                ...
                self.result = res;
                self.step = PedersenPointsXPhase::ComputePointStep2;
                vec![]
            }
            PedersenPointsXPhase::ComputePointStep2 => {
                let mut res = self.result;
                let point = self.pedersen_point;
                res = res * point
                    + Felt::from_hex_unchecked(
                ...
                self.result = res;
                self.step = PedersenPointsXPhase::ComputePointStep3;
                vec![]
            }
            PedersenPointsXPhase::ComputePointStep3 => {
                let mut res = self.result;
                let point = self.pedersen_point;
                res = res * point
                    + Felt::from_hex_unchecked(
                ...
                self.result = res;
                self.step = PedersenPointsXPhase::ComputePointStep4;
                vec![]
            }
            PedersenPointsXPhase::ComputePointStep4 => {
                let mut res = self.result;
                let point = self.pedersen_point;
                res = res * point
                    + Felt::from_hex_unchecked(
                ...
                self.result = res;
                self.step = PedersenPointsXPhase::ComputePointStep5;
                vec![]
            }
            PedersenPointsXPhase::ComputePointStep5 => {
                let mut res = self.result;
                let point = self.pedersen_point;
                res = res * point
                    + Felt::from_hex_unchecked(
                ...
                self.result = res;
                self.step = PedersenPointsXPhase::ComputePointStep6;
                vec![]
            }
            PedersenPointsXPhase::ComputePointStep6 => {
                let mut res = self.result;
                let point = self.pedersen_point;
                ...
                self.result = res;
                self.step = PedersenPointsXPhase::ComputePointStep7;
                vec![]
            }
            PedersenPointsXPhase::ComputePointStep7 => {
                let mut res = self.result;
                let point = self.pedersen_point;
                res = res * point
                    + Felt::from_hex_unchecked(
                ...
                self.result = res;
                self.step = PedersenPointsXPhase::ComputePointStep8;
                vec![]
            }
            PedersenPointsXPhase::ComputePointStep8 => {
                self.step = PedersenPointsXPhase::Done;
                let pedersen_points = self.result;
                stack.push_front(&pedersen_points.to_bytes_be()).unwrap();
                vec![]
            }
            PedersenPointsXPhase::Done => {
                vec![]
            }
        }
    }
