    ...
    fn execute<T: BidirectionalStack + ProofData + StarkVerifyTrait>(&mut self, stack: &mut T) -> Vec<Vec<u8>> {
        match self.step {
            TracesDecommitStep::PrepareOriginalTable => {
                ...
                stack.pop_front();
                let mut original_values = Vec::with_capacity(original_values_len.to_biguint().try_into().unwrap());
                ...
                let vector_commitment = VectorCommitment::new(vector_config, original_commitment_hash);
                let table_config = TableConfig {
                    n_columns: original_n_columns,
                    vector: vector_config,
                };
                let mut table_commitment = Commitment::new(table_config, vector_commitment);
                ...
                stack.push_front(&original_values_len.to_bytes_be()).unwrap();
                for value in original_values.iter().rev() {
                    stack.push_front(&value.to_bytes_be()).unwrap();
                }

                // Push witness count and authentications
                stack.push_front(&Felt::from(original_witness).to_bytes_be()).unwrap();
                VectorWitness::push_to_stack_static(stack, original_witness);
            ...
            TracesDecommitStep::PrepareInteractionTable => {
                // Read interaction commitment data
                ...
                stack.pop_front();
                let interaction_n_verifier_friendly = Felt::from_bytes_be_slice(stack.borrow_front());
                stack.pop_front();
                ...
                stack.pop_front();
                let mut interaction_values = Vec::with_capacity(interaction_values_len.to_biguint().try_into().unwrap());
                ...
                // Read interaction witness count
                let interaction_witness_count = VectorWitness::from_stack(stack);

                // Prepare interaction table decommit (like table_decommit(commitment.interaction, queries, decommitment.interaction, witness.interaction))
                ...
                let interaction_vector_commitment = VectorCommitment::new(interaction_vector_config, interaction_commitment_hash);
                let interaction_table_config = TableConfig {
                    n_columns: interaction_n_columns,
                    vector: interaction_vector_config,
                };
                let mut interaction_table_commitment = Commitment::new(interaction_table_config, interaction_vector_commitment);

                // Push data for interaction table decommit
                interaction_table_commitment.push_to_stack(stack);

                // Push queries (same as for original)
                let queries_len = Felt::from(self.queries_count);
                stack.push_front(&queries_len.to_bytes_be()).unwrap();
                // Note: In a real implementation, we would need to preserve queries from the first call
                // For now, we assume they are provided again on the stack

                // Push decommitment values
                stack.push_front(&interaction_values_len.to_bytes_be()).unwrap();
                for value in interaction_values.iter().rev() {
                    stack.push_front(&value.to_bytes_be()).unwrap();
                }

                // Push witness count and authentications
                stack.push_front(&Felt::from(interaction_witness_count).to_bytes_be()).unwrap();
                VectorWitness::push_to_stack_static(stack, interaction_witness_count);
